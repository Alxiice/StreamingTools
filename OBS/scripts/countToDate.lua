-- # Countdown Timer
-- 
-- Adapted from the countown script of OBS Studio
-- Except here we count down to a specific time of day
...
function update_remaining_seconds()
	local currentTime = os.date("*t")
	local current_time_seconds = (currentTime.hour * 3600) + (currentTime.min * 60) + currentTime.sec
	local seconds_left = target_seconds - current_time_seconds
	if seconds_left < 0 then
		seconds_left = 0
	end
	cur_seconds = seconds_left
end
...
function script_properties()
	...
	obs.obs_properties_add_int(props, "hour", "Start hour", 0, 23, 1)
	obs.obs_properties_add_int(props, "minute", "Start minute", 0, 59, 1)
...
function script_update(settings)
	...
	local mins = obs.obs_data_get_int(settings, "minute")
	target_seconds = (hour * 3600) + (mins * 60)
